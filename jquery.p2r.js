+(function _pulltorefresh__module($, document) {
  ...
  PullToRefresh.DEFAULTS = {
    orientation : "down", // define if is a pull-up-to-refresh or a
    // pull-down-to-refresh
    sensibility : 5, // number of pixels to each call of "move" event
    refresh : 200, // value in pixels to fire "refresh" event
    lockRefresh : false, // indicates that the user can pull up to get the
    // value "refresh"
    resetRefresh : true, // indicates that the "reset" function will be called
    // immediately when occur the event "refresh"
    autoInit : true, // indicates that the "PullToRefresh" object must be built
    // on startup "plugin"
  ...
  PullToRefresh.prototype.construct = function _pulltorefresh__construct() {
    var self = this;
    self.$element.on(PullToRefresh.events.start, self.proxy(self.onTouchStart, self)).on(PullToRefresh.events.move, self.proxy(self.onTouchMove, self)).on(PullToRefresh.events.end, self.proxy(self.onTouchEnd, self));

    if (self.options.simulateTouch) {
      self.$element.on(PullToRefresh.namespace('mousedown'), self.proxy(self.onTouchStart, self));
      $(document).on(PullToRefresh.namespace('mousemove'), self.$element, self.proxy(self.onTouchMove, self)).on(PullToRefresh.namespace('mouseup'), self.$element, self.proxy(self.onTouchEnd, self));
    }
  };
  ...
  PullToRefresh.prototype.transform = function _pulltorefresh__transform(style, value) {
    ...
    style.msTransform = style.MsTransform = style.MozTransform = style.OTransform = style.transform = 'translateY(' + value + 'px)';
  };
   ...
   *          style .style of Element
   * @param {string}
   *          ms css value to duration of transition
   * @method
   */
  PullToRefresh.prototype.transition = function _pullToRefresh__transition(style, ms) {
    style.webkitTransitionDuration = style.MozTransitionDuration = style.msTransitionDuration = style.OTransitionDuration = style.transitionDuration = ms;
  };
  ...
  PullToRefresh.prototype.remove_transition = function _pullToRefresh__remove_transition(style) {
    style.webkitTransitionDuration = style.MozTransitionDuration = style.msTransitionDuration = style.OTransitionDuration = style.transitionDuration = null;
  };
  ...
  PullToRefresh.prototype.remove_transform = function _pulltorefresh__remove_transform(style) {
    style.webkitTransform = style.msTransform = style.MsTransform = style.MozTransform = style.OTransform = style.transform = null;
  };
   ...
   *          event Event by mousedown or touchstart
   * @param {Boolean}
   *          isTouchEvent flag to indicate a touch event
   ...
   * 
   * @param {MouseEvent|TouchEvent}
   *          event Original event fired by DOM
   * @method
   */
  PullToRefresh.prototype.onTouchStart = function _pulltorefresh__ontouchstart(event) {
    var isTouchEvent = event.type === 'touchstart', axis = this.getAxis(event, isTouchEvent);
   ...
   * 
   * @param {MouseEvent|TouchEvent}
   *          event Original move event fired by DOM
   * @method
   */
  PullToRefresh.prototype.onTouchMove = function _pulltorefresh__ontouchmove(event) {

    var isTouchEvent = event.type === 'touchmove', delta, step, percentage, axis;

    // if not touched or hasTouchEvent and the eventType is a desktop event
    // cancel the move
    ...
    if (delta < 0 && this.options.orientation == 'down')
      return;
    if (delta >= 0 && this.options.orientation == 'up')
      return;
   ...
   * 
   * @param {MouseEvent|TouchEvent}
   *          event Original end event fired by DOM
   * @method
   */
  PullToRefresh.prototype.onTouchEnd = function PullToRefresh__onTouchEnd(event) {
    ...
    if (delta > 20) {
      console.log("onTouchEnd detected dragging. Reset.");
      ...
      console.log("onTouchEnd detected no dragging. Allow click.");
    }
